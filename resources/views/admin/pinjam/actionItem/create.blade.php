<x-app-layout>
    <div class="m-10 bg-white p-10 rounded-lg">
        <span class="font-bold text-4xl">Item Pinjam</span>

        @php
            $idKota = Auth::user()->id_kota;
        @endphp
        <div class="mt-10 mx-4">
            <form method="GET" action="{{ $idPinjam }}">
                    <div class="flex items-center">
                        <div class="relative pl-3">
                            <select name="id_kota"
                                class="bg-white border border-gray-300 text-gray-900 w-full rounded-md">
                                <option value=""> Pilih Kota/Kab Pemberi </option>
                                @foreach ($kotas as $kota)
                                    <option value="{{ $kota->id }}"
                                        @if ($kota->id == old('id_user')) selected @endif>
        ...
        <div class="mt-3">
            <div class="block overflow-x-auto p-8 text-dark">
                <form method="POST" action="/adminpinjamItem" enctype="multipart/form-data">
                    @csrf
                    <input hidden name="idPinjam" value="{{ $idPinjam }}" placeholder="">
                    <div class="mb-6">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Pilih
                            Peralatan</label>
